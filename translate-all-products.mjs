// Tüm ürünleri FR, DE, JA diline çeviren script
// Çalıştırmak için: node translate-all-products.mjs
...
async function fetchProducts() {
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/products?select=id,title,description,title_fr,title_de,title_ja&is_active=eq.true&order=display_order.asc`,
...
async function main() {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    const missing = TARGET_LANGS.filter(l => !product[`title_${l}`]);

    if (missing.length === 0) {
      console.log(`[${i+1}/${products.length}] "${product.title}" — tüm diller mevcut, atlanıyor.`);
      continue;
    }

    console.log(`[${i+1}/${products.length}] "${product.title}" → çevriliyor: ${missing.join(', ')}`);
    ...
    // Ürünler arası bekleme (rate limit)
